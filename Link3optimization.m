...
g=9.81;
linear_weight_density_canopy = 0.0015*g;%N/cm          %Linear density of all the links except 3 and 5
linear_weight_density_structure = 0.005*g;%N/cm          %Linear density of all the links except 3 and 5
lead_linear_density = 0.005;

%% Weights of links in Newtons
w1 = linear_weight_density_structure * L1;                                                                                   
w2 = linear_weight_density_structure * L2;                                              
w3 = 16*g;                                               
w4 = linear_weight_density_structure * L4;                                                
w5 = 15.3;                                              
w6 = linear_weight_density_structure * (L6 + L4*(1-GH_ratio) + sqrt(L6^2 + (L4*(1-GH_ratio))^2- 2*L6*L4*(1-GH_ratio)*cos(theta_IEJ)));                                                
w7 = linear_weight_density_structure * L7;                                                
w8 = linear_weight_density_structure * L8;                                               
w9 = lead_linear_density * L9;                                                
w10A=linear_weight_density_canopy * (L10A);
w10B=linear_weight_density_canopy * (L10B);  
w10C=linear_weight_density_canopy * (L10C);             
w11 =linear_weight_density_canopy * L11;                                              
w12 =linear_weight_density_canopy * L12;                                               
w13 =linear_weight_density_canopy * L13;
w14 =w13;
...
%%     %% Find Moments about Ground link
    %By making the center of gravity close to the ground link, the applied
    %force on link 2 reduces
    Moment(i) = 0;
    Moment(i) = Moment(i) + w1*avg_001x(i);
    Moment(i) = Moment(i) + w2*avg_002x(i);
    Moment(i) = Moment(i) + w3*avg_003x(i);
    Moment(i) = Moment(i) + w4*avg_004x(i);
    Moment(i) = Moment(i) + w5*avg_005x(i);
    Moment(i) = Moment(i) + w6*avg_06ax(i);
    Moment(i) = Moment(i) + w6*avg_06bx(i);
    Moment(i) = Moment(i) + w6*avg_06cx(i);
    Moment(i) = Moment(i) + w7*avg_007x(i);
    Moment(i) = Moment(i) + w8*avg_008x(i);
    Moment(i) = Moment(i) + w9*avg_009x(i);
    Moment(i) = Moment(i) + w10A*avg_10ax(i);
    Moment(i) = Moment(i) + w10B*avg_10bx(i);
    Moment(i) = Moment(i) + w10C*avg_10cx(i);
    Moment(i) = Moment(i) + 4*w11*avg_011x(i);
    Moment(i) = Moment(i) + 4*w12*avg_012x(i);
    Moment(i) = Moment(i) + 4*w13*avg_013x(i);
    Moment(i) = Moment(i) + 4*w14*avg_014x(i);
    Moment(i) = Moment(i) / w_tot;
    hold off;
    moment_arm(i) = -L2*cos(theta_2(i))*0.01;
    input_force(i) = Moment(i)/moment_arm(i);
end

figure(2);                         %create new figure
set(2,'WindowStyle','Docked')      %dock the figure
plot(t, input_force);           % draw a point at y_Avg
title("Input Force at link 2");
xlabel("Time(s)");
...
if false
    ...
    plot(canopy_angle/deg-90,F1);
    title("Force vs Canopy angle");
    xlabel("Canopy angle");
    ylabel("Force including link weight")
end
