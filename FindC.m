...
for i = 1:length(theta_2)
    ...
    y_avg_canopy(i) = y_avg_canopy(i) / (W_canopy);


    %Two force analysis calculation of vertical force input on link 5
end
for i = 2:length(theta_2)
    C(i) = (y_avg_canopy(i)-y_avg_canopy(1))/(Sy(i)-Sy(1));
end
disp("The C ratio is:")
disp(C(5));
plot(t(5:length(C)),C(5:length(C)));
title("This graph shows that C is constant")
xlabel('t(s)')
ylabel('C(unitless ratio)')
axis equal;
